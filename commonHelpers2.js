import"./assets/sign-in-05cc8f21.js";import"./assets/vendor-313c7975.js";const n=JSON.parse(localStorage.getItem("bookList"))||[],a=document.querySelector(".shopping-list");function r(t,o){const s=n.slice(t,o).map(i=>`
 ...
 <svg class="trash-delete" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
    ...
    `).join(""),p=document.querySelector(".shopping-title");p.style.paddingBottom="40px",a.innerHTML=s}function c(t){const o=t.find(s=>s.name==="Amazon"),e=t.find(s=>s.name==="Apple Books");return`
  ...
  `}r(0,n.length);a.addEventListener("click",g);function g(t){const o=t.target.closest(".delete-book");if(o){const e=o.closest(".shopping-item");if(e){const s=e.id;a.removeChild(e),d(s)}}}function d(t){const e=(JSON.parse(localStorage.getItem("bookList"))||n).filter(s=>s._id!==t);localStorage.setItem("bookList",JSON.stringify(e)),n.length=0,Array.prototype.push.apply(n,e),n.length===0&&(l(),setTimeout(()=>{location.reload()},1e3))}n.length===0&&(l(),window.addEventListener("storage",t=>{t.key==="bookList"&&JSON.parse(t.newValue).length===0&&location.reload()}));function l(){const t=`
