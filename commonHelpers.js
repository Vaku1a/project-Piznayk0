...
/>
            ...
            </div>`,t.booksPart.querySelectorAll(".book-cards-list-item").forEach(r=>r.style.display="block")}).catch(s=>{console.error(s)}).finally(s=>t.loaderForAllCategories.style.display="none")}function P(o){const e=o.map(({list_name:a})=>`<li class="categories-list-item" data-category-name="${a}">${a}</li>`).sort((a,s)=>a.localeCompare(s));return e.unshift('<li class="categories-list-item active" data-category-name="All categories">All categories</li>'),e.join("")}function p(o){const e=o.split(" ");if(e.includes("and")){const r=e.indexOf("and"),l=e.slice(0,r+1).join(" "),b=e.slice(r+1).join(" ");return`<h2 class="books-part-title">${l} <span class="books-part-title-span">${b}</span></h2>`}if(e.length>3){const r=e.slice(0,e.length/2).join(" "),l=e.slice(e.length/2).join(" ");return`<h2 class="books-part-title">${r} <span class="books-part-title-span">${l}</span></h2>`}const a=e.pop();return`<h2 class="books-part-title">${e.join(" ")} <span class="books-part-title-span">${a}</span></h2>`}function A(o){return o.map(({_id:e,book_image:a,author:s,title:r})=>`<li class="book-cards-list-item" data-book-id="${e}">
            <img
            class="book-card-img"
            src="${a}"
            alt="${r}"            
